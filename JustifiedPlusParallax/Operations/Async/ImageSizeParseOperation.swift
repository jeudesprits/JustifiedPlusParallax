...
final class ImageSizeParseOperation: AsyncOperation {
  ...
  override func main() {
    let operation = ImageBytesFetcherOperation(session: session, url: url, range: range)
  ...
  private func parse() {
    guard let marker = parseMarker(by: data[offset]) else { state = .isFinished; return }
